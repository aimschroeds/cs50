...
int main(int argc, string argv[])
{
    ...
    {
        if ((plain_txt[i] >= 'a' && plain_txt[i] <= 'z') || (plain_txt[i] >= 'A' && plain_txt[i] <= 'Z'))
        {
            for (int j = 0; j < 26; j++)
            {
                if (plain_txt[i] == alpha[j])
                {
                    printf("%c", toupper(cypher_txt[j]));
                }
                else if (plain_txt[i] == tolower(alpha[j]))
                {
                    printf("%c", tolower(cypher_txt[j]));
                }
                else
                {
                    continue;
