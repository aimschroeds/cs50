...
void lock_pairs(void)
{
    ...
    for (int i = 0; i < pair_count; i++)
    {
       printf("About to call pair tree for %i - %i\n", winners[i].winner, winners[i].loser);
       pair_tree(winners[i].winner, winners[i].loser);
    }
...
void pair_tree(int x, int y)
{
    for (int j = 0; j < candidate_count; j++)
    {
        if (locked[y][j])
        {
            ...
            {
                locked[x][y] = false;
                printf("Discarded: Winner - %i, Loser - %i\n", x, y);
                return;
            }
            else
            {
        ...
        {
            locked[x][y] = true;
            printf("Locked: Winner - %i, Loser - %i\n", x, y);
            return;
        }
    }
}
